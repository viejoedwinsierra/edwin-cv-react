...
export function TopBar({ lang, meta, activeView, onNavigate, variant, onVariantChange, onToggleLanguage, onPrint }) {
  const nav = [
  ...
  return (
    <header className="topbar no-print">
      <nav className="quick-nav" aria-label={lang === 'es' ? 'Navegación del CV' : 'CV navigation'}>
        {nav.map(([id, label]) => (
          <button
            type="button"
            key={id}
      ...
      <div className="toolbar">
        ...
        <button type="button" className="toolbar-link" onClick={onToggleLanguage}>{meta.switchLabel}</button>
        <button type="button" onClick={onPrint} className="print-button">{meta.printLabel}</button>
      </div>
    </header>
  );
}
